...
if (isset($submit) && $submit == 'create') {
    ...
    if ($ext == 'jpg' || $ext == 'png' || $ext == 'jpeg') {
        ...
        $query = "INSERT INTO emp(id,name,email,profilepic,number) values('$empid','$empname','$empemail','$profilepic_path','$emppho')";
    ...
    if ($_FILES['profilepic']['size'] == 0) {
        $query = "UPDATE emp set name='$empname',email='$empemail',number='$emppho' where id='$empid'";
        $result = mysqli_query($link, $query);
        if ($result) {
            echo 'success';
